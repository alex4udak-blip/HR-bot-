...
class CallProcessor:
    ...
    async def _analyze(self, transcript: str) -> dict:
        ...
        prompt = f"""Проанализируй транскрипт созвона и создай подробный анализ:

1. SUMMARY - развёрнутое резюме разговора:
   - Контекст и цель встречи (1 абзац)
   - Основные обсуждаемые темы с деталями (2-3 абзаца)
   - Итоги и следующие шаги (1 абзац)

2. KEY_POINTS - ключевые моменты (7-12 пунктов):
   - Каждый пункт должен быть информативным (1-2 предложения)
   - Включи конкретные детали, цифры, имена если они упоминались

3. ACTION_ITEMS - конкретные задачи и договорённости:
   - Укажи кто ответственный если понятно из контекста
   - Укажи сроки если они обсуждались
...
{{
  "summary": "развёрнутое резюме в нескольких абзацах...",
  "key_points": ["подробный пункт 1", "подробный пункт 2", ...],
  "action_items": ["конкретная задача 1", "конкретная задача 2", ...]
}}"""
