...
# File operations logger (must be defined before functions that use it)
...
# Supported document formats for LibreOffice conversion to PDF
DOC_EXTENSIONS = {'.doc', '.docx', '.odt', '.rtf'}


async def convert_doc_to_pdf(doc_path: Path) -> Optional[Path]:
    """
    Convert DOC/DOCX/ODT/RTF to PDF using LibreOffice.
    Returns path to generated PDF or None on failure.
    LibreOffice is installed in Docker (libreoffice-writer-nogui).
    """
    try:
        # Create a temp dir for LibreOffice output
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            file_logger.info(f"DOC_TO_PDF: running {' '.join(cmd)}")
            ...
            if proc.returncode != 0:
                file_logger.error(
                    f"DOC_TO_PDF: LibreOffice failed (rc={proc.returncode}): "
            ...
            if not pdf_files:
                file_logger.error("DOC_TO_PDF: no PDF generated by LibreOffice")
                return None

            # Move PDF to same directory as original doc
            src_pdf = pdf_files[0]
            dst_pdf = doc_path.with_suffix('.pdf')
            shutil.copy2(str(src_pdf), str(dst_pdf))

            file_logger.info(f"DOC_TO_PDF: success → {dst_pdf} ({dst_pdf.stat().st_size} bytes)")
            return dst_pdf

    except asyncio.TimeoutError:
        file_logger.error("DOC_TO_PDF: LibreOffice timed out (120s)")
        return None
    except Exception as e:
        file_logger.error(f"DOC_TO_PDF: unexpected error: {e}")
        return None
...
async def upload_entity_file(
    ...
    if file_type_enum == EntityFileType.resume:
        ...
        if file_extension == '.pdf':
            pdf_for_conversion = file_path
        elif file_extension in DOC_EXTENSIONS:
            # DOC/DOCX → PDF via LibreOffice, then PDF → images
            file_logger.info(
                f"DOC resume detected for entity {entity_id} "
                f"(ext={file_extension}), converting via LibreOffice..."
            )
            try:
                pdf_for_conversion = await convert_doc_to_pdf(file_path)
            except Exception as e:
                file_logger.error(
                    f"DOC-to-PDF conversion failed for entity {entity_id}: {e}"
                )
        ...
        if pdf_for_conversion:
            ...
            try:
                ...
                file_logger.error(
                    f"PDF-to-image conversion failed for entity {entity_id}: {e}"
                )
            finally:
                # Clean up intermediate PDF if it was converted from DOC
                if file_extension in DOC_EXTENSIONS and pdf_for_conversion and pdf_for_conversion.exists():
                    try:
                        pdf_for_conversion.unlink()
                    except Exception:
                        pass
