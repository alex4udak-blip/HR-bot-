...
## Railway
- **URL:** https://hr-bot-production-c613.up.railway.app
- **Auto-deploy:** при push в main
- **Время деплоя:** ~3-5 мин

### ВАЖНО: Как читать Railway Deployments
В списке деплоев на Railway:
- **ACTIVE (зелёный сверху)** — это ТЕКУЩИЙ работающий деплой на проде
- **BUILDING (ниже)** — это НОВЫЙ деплой, который ещё собирается
- Новые деплои появляются СНИЗУ и поднимаются вверх когда становятся активными
- НЕ путай! Зелёный сверху ≠ последний. Последний = тот что BUILDING

## Проверка на проде
После merge:
1. Подожди 3-5 мин (Railway деплоит)
2. Проверь что новый деплой стал ACTIVE (зелёный, сверху)
3. Открой URL через Chrome extension
4. Проверь функционал
5. Обнови WORK_LOG.md
