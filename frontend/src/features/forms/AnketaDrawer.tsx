      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-3xl bg-white shadow-xl flex flex-col">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {step === 'responses' && <ResponsesStep dispatches={dispatches} />}
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
...
function ResponsesStep({ dispatches }: { dispatches: FormDispatchInfo[] }) {
  if (dispatches.length === 0) {
    return <div className="p-8 text-center text-gray-400 text-sm">Пока нет отправленных анкет</div>;
  }
  return (
