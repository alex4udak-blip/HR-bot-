...
class CallProcessor:
    ...
    async def _analyze(self, transcript: str) -> dict:
        ...
        prompt = f"""Ты опытный бизнес-аналитик. Проанализируй транскрипт созвона и создай ГЛУБОКИЙ структурированный анализ.

ВАЖНО: Не просто пересказывай что обсуждалось - делай настоящий анализ с инсайтами и выводами!

Создай анализ по следующей структуре:

## SUMMARY (развёрнутое резюме):

**СУТЬ РАЗГОВОРА** (2-3 предложения):
Ключевая цель встречи и её главный результат.

**ОСНОВНАЯ ТЕМА:**
Главная тема разговора с деталями.

**ПОДТЕМЫ:**
Перечисли 2-5 подтем которые обсуждались.

**К ЧЕМУ ПРИШЛИ (договорённости):**
Конкретные решения и договорённости, которые были достигнуты.

**К ЧЕМУ НЕ ПРИШЛИ (открытые вопросы):**
Вопросы которые остались нерешёнными или требуют дополнительного обсуждения.

**ИНСАЙТЫ И ВЫВОДЫ:**
- Важные наблюдения о ситуации
- Скрытые риски или возможности
- Что можно понять между строк

**ОБЩАЯ ОЦЕНКА:**
Насколько продуктивным был разговор, что можно улучшить.

## KEY_POINTS (8-15 ключевых моментов):
Каждый пункт должен быть конкретным, с деталями, цифрами, именами. Не общие фразы, а конкретика!

## ACTION_ITEMS (задачи и следующие шаги):
- Кто ответственный
- Что нужно сделать
- Сроки если обсуждались
...
{{
  "summary": "Полное форматированное резюме с разделами (используй **жирный** для заголовков, переносы строк для структуры)",
  "key_points": ["Конкретный момент 1 с деталями", "Конкретный момент 2", ...],
